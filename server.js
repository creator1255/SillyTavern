...
app.post("/getstatus_openai", jsonParser, function (request, response_getstatus_openai = response) {
    ...
    const emptyObject = {"none":"none"};
    response_getstatus_openai.send(emptyObject);
    return;
...
app.post("/generate_openai", jsonParser, function (request, response_generate_openai) {
    ...
    // flatten OpenAI chat completion payload into a single string for nat.dev
    let messagePayload = [];
    ...
    messagePayload = messagePayload.join("\n");
    ...
    const config = {
        ...
        data: {
            ...
            models: [{
                "name": `${provider}:${request.body.model}`,
                "enabled": true,
                "selected": true,
                "provider": provider,
                "tag": `${provider}:${request.body.model}`,
                "parameters": {
                    "temperature": request.body.temperature,
                    "maximumLength": request.body.max_tokens,
                    "topP": 1,
                    "topK": 1,
            ...
            if (response.status <= 299) {
                if (request.body.stream) {
                    ...
                    for (let i = 0; i < messages.length; i++) {
                        ...
                        const splitedMessages = messages[i].split("\n");
                        if(splitedMessages[0] != "event:infer"){
                            continue;
                        }
                        const json = JSON.parse(splitedMessages[1].substring(5));
                        fullMessages = fullMessages + json.message;
                    ...
                    console.log(natDevData);
                    response_generate_openai.send(natDevData);
                }
...
app.post("/tokenize_openai", jsonParser, function (request, response_tokenize_openai = response) {
    ...
    let tokenizer;
    if(request.query.model.includes("claude")){
        // I don't know which tokenizer to use :p
        tokenizer = tiktoken.encoding_for_model("gpt-3.5-turbo");
    }
    else {
        tokenizer = tiktoken.encoding_for_model(request.query.model);
    }
